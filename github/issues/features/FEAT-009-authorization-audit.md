...
**Milestone:** M2 &nbsp;·&nbsp; **Priority:** Critical &nbsp;·&nbsp; **Effort:** ~2 days &nbsp;·&nbsp; **Status:** Not Started

## Purpose

Enforce the entry-vs-verification safety boundary and audit every clinical write.
...
- `ADR-0008` (to be drafted via the `plan`/`architect` skill if a load-bearing decision surfaces)
...
- [ ] A bench-role user is refused when attempting to verify a result they entered
- [ ] Every clinical mutation produces exactly one audit_event row with correct actor, tenant, and timestamp

## Tasks

- [ ] `TASK-032` — Role model + capability checks (enter != verify) (M (1 day))
- [ ] `TASK-033` — Audit interceptor on every clinical mutation (M (1 day))
...
- [ ] Proposal file: `docs/plans/feat-009-authorization-audit.md`
...
- **Project fields:** Type=Feature, ID=FEAT-009, Epic=EPIC-002, Priority=Critical, Effort=2d, Area=backend, Milestone=M2, Status=Not Started
