...
id: FEAT-006
type: feature
title: "Order, specimen & tenancy spine"
epic: EPIC-001
milestone: M1
priority: Critical
effort_days: 5
...
The aggregates results attach to, structurally isolated per tenant.
...
- `ADR-0006` (to be drafted via the `plan`/`architect` skill if a load-bearing decision surfaces)
...
- [ ] Row-Level Security isolation test proves tenant A cannot read tenant B's rows under any query shape
- [ ] audit_event hash chain is broken and detected when a row is tampered with
- [ ] Specimen aliquot/parent lineage is queryable

## Tasks

- [ ] `TASK-023` — Migration: order, ordered_test, specimen, M:N link (M (1 day))
- [ ] `TASK-024` — tenant_id + RLS policies on all tenant tables (L (2 days))
- [ ] `TASK-025` — audit_event table with hash chain + writer (M (1 day))
...
- [ ] Proposal file: `docs/plans/feat-006-order-specimen-tenancy-spine.md`
...
- **Project fields:** Type=Feature, ID=FEAT-006, Epic=EPIC-001, Priority=Critical, Effort=5d, Area=db, Milestone=M1, Status=Not Started
