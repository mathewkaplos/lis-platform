...
Tests, analytes, units, and reference ranges exist as data, not code.
...
- `ADR-0004` (to be drafted via the `plan`/`architect` skill if a load-bearing decision surfaces)
...
- [ ] LOINC-coded analyte rows insert and query correctly via (system,code,version,display) tuples
- [ ] A CBC panel resolves to its constituent analytes in a single query
- [ ] Reference range resolves correctly for age/sex/method dimensions on a seeded case
- [ ] Design partner's real chemistry test menu is seeded and queryable

## Tasks

- [ ] `TASK-016` — Migration: analyte, unit, code_system_value (M (1 day))
- [ ] `TASK-017` — Migration: test_definition, test_analyte, panel (M (1 day))
- [ ] `TASK-018` — Migration: reference_range (sex/age/method/criticals/versioned) (L (2 days))
- [ ] `TASK-019` — Seed design partner's real chemistry catalog (M (1 day))
...
- [ ] Proposal file: `docs/plans/feat-004-catalog-metadata-model.md`
...
- **Project fields:** Type=Feature, ID=FEAT-004, Epic=EPIC-001, Priority=Critical, Effort=5d, Area=db, Milestone=M1, Status=Not Started
