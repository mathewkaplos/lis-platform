...
| HL7 v2 / FHIR R4 | **2 — Automated-tested** | 18 e2e-spec-covered routes exist; zero live-browser or live-message verification found anywhere in this repo's history, confirmed again this pass | No change — still the single largest proof gap found |
| Analyzer integration (`gateway-ingest`) | **2 — Automated-tested** | Real correlation/idempotency service, unit/e2e tested; `FEAT-027` (a real analyzer driver) still open | No change |
...
| Synoptic protocol engine | **6 — Pilot-proven** (for 2 of 7 protocols) / **1 — Implemented** (for the other 5) | Breast + Colorectal live-filled multiple times; Lung/Prostate/Cervical-Cytology/Colon-Rectum-CAP/Breast-Biomarker never opened by a human, per the pilot guide's own scorecard | No change, restated with the level model applied per-protocol rather than as one blended number |
